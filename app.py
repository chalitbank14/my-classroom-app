...
Version: 5.3.1-Stable (Hotfix: Image Generation Variable)
...
Thai language typography rendering.
"""
...
class AppConfig:
    ...
    APP_VERSION: str = "5.3.1-Enterprise"
...
class GoogleSheetsRepository:
    ...
    def update_group_record(self, room: str, old_name: str, new_name: str, new_members: str, current_df: pd.DataFrame) -> bool:
        if new_name != old_name:
             ...
             if duplicate_mask.any(): return False

        target_mask = (current_df['Room'] == room) & (current_df['GroupName'] == old_name)
        if not target_mask.any(): return False
    ...
    def process_xp_transaction(self, room: str, target_groups: List[str], amount: int, reason: str, 
        ...
        for group_name in target_groups:
            ...
            if mask.any():
                ...
                try: history_list = json.loads(current_df.at[idx, 'HistoryLog'])
                except: history_list = []
    ...
    def apply_history_override(self, room: str, group_name: str, new_history_df: pd.DataFrame, 
        ...
        try: history_sorted_asc = sorted(history_list, key=lambda x: x.get('ts', ''))
        except: history_sorted_asc = history_list
...
class GraphicsEngine:
    ...
    def render_leaderboard_image(self, room_name: str, df: pd.DataFrame, rank_manager: RankManager) -> bytes:
        ...
        # Correctly calculate canvas height based on rows
        canvas_height = (
        ...
        for i, row in leaderboard_data.iterrows():
            ...
            # Col 2: Info (Critical Spacing)
            content_x_start = card_x_start + 260
            content_max_width = 650
            
            # Explicit Y-Grid System to prevent overlaps
        ...
        # --- FOOTER ---
        # FIX: Used canvas_height (calculated earlier) instead of undefined total_height
...
class UIManager:
    ...
    def render_sidebar(self):
        with st.sidebar:
            ...
            selected_room = st.selectbox("Select Active Class", ["ม.1/1", "ม.1/2", "ม.1/3", "ม.1/4", "ม.1/10"])
            st.divider()
    ...
    def run(self):
        ...
        st.markdown(f"""<div class="hero-banner"><div><h1>{selected_room}</h1></div><div style="font-size:3rem; font-weight:800;">{len(room_df)} Teams</div></div>""", unsafe_allow_html=True)
        
        t1, t2, t3, t4, t5 = st.tabs(["⚡ Command", "🏆 Leaderboard", "📈 Analytics", "ℹ️ Privileges", "🛠️ Management"])
        
        with t1:
            if room_df.empty: st.info("Create groups in Management tab first.")
            else:
                target = st.multiselect("Select Teams", room_df['GroupName'].unique())
                c1, c2 = st.columns(2)
                def act(r, a):
                    if target: 
                        self.db.process_xp_transaction(selected_room, target, a, r, all_df, self.badge_sys)
        ...
        with t2:
            if st.button("✨ Generate Image", type="primary"):
                img = self.gfx.render_leaderboard_image(selected_room, room_df, self.rank_mgr)
                st.image(img)
        ...
        with t5:
            with st.form("new"):
                ...
                if st.form_submit_button("Create") and n:
                    self.db.create_group_record(selected_room, n, m, all_df)
                    st.rerun()
            
            delt = st.selectbox("Delete", ["-"]+list(room_df['GroupName'].unique()))
            if delt != "-" and st.button("Delete"):
                self.db.delete_group_record(selected_room, delt, all_df)
                st.rerun()
